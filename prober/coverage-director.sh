...
#
# coverage-director.sh -- OPT-IN generator for prober/coverage-map.json, a
...
# prober/*.d (verify-impact:79-85). Only ever write a regular file.
...
for src in *_test.c; do
    ...
    # output. -r . roots relative paths at prober/; --filter narrows to the
    ...
    # into "prober/F<TAB>N<TAB>test_name" rows for every line with count > 0.
    jq -r --arg test "$test_name" '
        .files[]
        | .file as $f
        | .lines[]
        | select(.count > 0)
        | "prober/\($f)\t\(.line_number)\t\($test)"
    ' "$json_out" >>"$rows"
