...
#
# TAP self-test for prober/coverage-director.sh and its consumer, the
# coverage-map.json layer in prober/verify-impact.
#
...
# in its own prober/ directory so coverage-director.sh's glob-based discovery
...
a_only_fn1=1
if jq -e '.["prober/lib.c"]["1"] // [] | index("a_test")' "$DIR1/coverage-map.json" >/dev/null 2>&1 \
   && ! jq -e '.["prober/lib.c"]["2"] // [] | index("a_test")' "$DIR1/coverage-map.json" >/dev/null 2>&1; then
    :
else
    a_only_fn1=0
fi
...
flipped=1
if jq -e '.["prober/lib.c"]["2"] // [] | index("a_test")' "$DIR2/coverage-map.json" >/dev/null 2>&1 \
   && ! jq -e '.["prober/lib.c"]["1"] // [] | index("a_test")' "$DIR2/coverage-map.json" >/dev/null 2>&1; then
    :
else
    flipped=0
fi
...
printf '{"prober/json.c": {"1": ["json_test"]}}\n' >"$REPO/prober/coverage-map.json"
git -C "$REPO" add -A
...
set +e
out="$(cd "$REPO" && ./prober/verify-impact --base "$BASE_SHA" --explain)"
s=$?
set -e
...
printf '{"prober/json.c": {"1": ["other_test"]}}\n' >"$REPO/prober/coverage-map.json"
git -C "$REPO" add -A
...
set +e
out2="$(cd "$REPO" && ./prober/verify-impact --base "$OTHER_BASE" --explain)"
s2=$?
set -e
other_ok=1
[ "$s2" -eq 0 ] || other_ok=0
...
printf '{"prober/json.c": {"99": ["json_test"]}}\n' >"$REPO/prober/coverage-map.json"
git -C "$REPO" add -A
...
set +e
cd "$REPO" && ./prober/verify-impact --base "$UNCOV_BASE" >/dev/null 2>&1
s3=$?
cd - >/dev/null
set -e
ok "$((s3 != 0 ? 0 : 1))" \
