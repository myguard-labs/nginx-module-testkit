...
#
# TAP self-test for ci/prober/pr-impact (P2-C).
#
...
#
# Layout: ci/prober/{lib.c,lib.h,lib_test.c,verify-impact,impact.map,pr-impact,
#         mutate.sh,build.sh,test.sh}, ci/prober/fuzz/{fuzz_lib.c,fuzz_standalone.c,
...
#                            propagate as "not ok".
REPO="$WORK/repo"
mkdir -p "$REPO/ci/prober/fuzz/corpus/lib"
git -C "$REPO" init -q
...
cat >"$REPO/ci/prober/lib_test.c" <<'EOF'
#include <stdio.h>
#include <stdlib.h>
#include "lib.h"
int main(void) {
...
cat >"$REPO/ci/prober/fuzz/fuzz_lib.c" <<'EOF'
#include <stddef.h>
#include <stdint.h>
#include <stdlib.h>
#include "../lib.h"
...
EOF
cp "$PWD/fuzz/fuzz_standalone.c" "$REPO/ci/prober/fuzz/fuzz_standalone.c"
printf 'safe seed\n' >"$REPO/ci/prober/fuzz/corpus/lib/seed"
...
cat >"$REPO/ci/prober/build.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")"
CC="${CC:-cc}"
...
EOF
chmod +x "$REPO/ci/prober/build.sh"
...
cat >"$REPO/ci/prober/mutate.sh" <<'MUTEOF'
#!/usr/bin/env bash
set -uo pipefail
cd "$(dirname "$0")" || exit 1
FILTER="${1:-}"
...
exit "$rc"
MUTEOF
chmod +x "$REPO/ci/prober/mutate.sh"
...
cat >"$REPO/ci/prober/test.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")"
./build.sh >/dev/null
./lib_test
EOF
chmod +x "$REPO/ci/prober/test.sh"

cp "$VERIFY_IMPACT_SRC" "$REPO/ci/prober/verify-impact"
chmod +x "$REPO/ci/prober/verify-impact"

cat >"$REPO/ci/prober/impact.map" <<'EOF'
PRIMARY	ci/prober/lib.c	lib_test	fast
SEMANTIC	ci/prober/fuzz/fuzz_lib.c	fuzz/fuzz_lib	fast
SEMANTIC	ci/prober/lib.c	fuzz/fuzz_lib	fast
SEMANTIC	ci/prober/fuzz/corpus/lib	fuzz/fuzz_lib	fast
SEMANTIC	ci/prober/lib.c	mutate:add_one	fast
SEMANTIC	ci/prober/build.sh	lib_test	fast
SEMANTIC	ci/prober/mutate.sh	lib_test	fast
SEMANTIC	ci/prober/lib_test.c	lib_test	fast
EOF
...
make_pr_impact_copy "$REPO/ci/prober/pr-impact"
...
run_pri() {
    ( cd "$REPO/ci/prober" && ./pr-impact --base "$BASE_SHA" --budget 45 \
...
cat >"$REPO/ci/prober/shellunit_test.sh" <<'SHTEST'
#!/usr/bin/env bash
set -euo pipefail
echo "1..1"
...
exit 0
SHTEST
chmod +x "$REPO/ci/prober/shellunit_test.sh"
printf 'PRIMARY\tci/prober/shellunit_test.sh\tshellunit_test.sh\tfast\n' >>"$REPO/ci/prober/impact.map"
git -C "$REPO" add -A
...
sed -i 's/^echo "1\.\.1"$/echo "1..1"   # selected by its own PRIMARY row/' "$REPO/ci/prober/shellunit_test.sh"
git -C "$REPO" add -A
git -C "$REPO" commit -q -m "no-op edit to the shell suite -- selects itself"
set +e
LAST_OUT="$( cd "$REPO/ci/prober" && ./pr-impact --base "$BASE_SH" --budget 45 \
...
sed -i 's/^exit 0$/exit 1/' "$REPO/ci/prober/shellunit_test.sh"
git -C "$REPO" add -A
...
BASE_P2="$BASE_SHA"
set +e
LAST_OUT="$( cd "$REPO/ci/prober" && ./pr-impact --base "$BASE_P2" --budget 45 \
...
printf 'Xtrigger\n' >"$REPO/ci/prober/fuzz/corpus/lib/crash_seed"
git -C "$REPO" add -A
...
if command -v valgrind >/dev/null 2>&1; then
    set +e
    LAST_OUT="$( cd "$REPO/ci/prober" && ./pr-impact --base "$BASE_P3" --budget 45 \
...
EOF
    ...
    LAST_OUT="$( cd "$REPO/ci/prober" && ./pr-impact --base "$BASE_SHA" --budget 45 \
...
printf '{"ci/prober/lib.c": {"2": ["lib_test"]}}\n' >"$REPO/ci/prober/coverage-map.json"
git -C "$REPO" add -A
...
set +e
LAST_OUT="$( cd "$REPO/ci/prober" && ./pr-impact --base "$BASE_P4" --budget 45 \
...
printf '{"ci/prober/lib.c": {"1": ["lib_test"]}}\n' >"$REPO/ci/prober/coverage-map.json"
git -C "$REPO" add -A
...
set +e
LAST_OUT="$( cd "$REPO/ci/prober" && ./pr-impact --base "$FAULT_P4" --budget 45 \
...
set +e
LAST_OUT="$( cd "$REPO/ci/prober" && ./pr-impact --base "$BASE_P5" --budget 45 \
...
set +e
LAST_OUT="$( cd "$REPO/ci/prober" && ./pr-impact --base "$FAULT_P5_BASE" --budget 45 \
...
set +e
LAST_OUT="$( cd "$REPO/ci/prober" && ./pr-impact --base "$DOCS_PARENT" --budget 45 \
...
# exit 0".
set +e
( cd "$REPO/ci/prober" && ./verify-impact --base "$BASE_SHA" ) >/dev/null 2>/dev/null
VI_EXPECTED=$?
set -e
