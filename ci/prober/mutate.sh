...
refresh_ref_module_for_scenario () {
    ...
    # mutate.sh runs with cwd = ci/prober/ (see the `cd` at the top of this
    # file), but PROBER_ROOT -- both CI's explicit github.workspace and lib.sh's
    # own BASH_SOURCE-derived default -- is the REPO ROOT, and that is where
    # prober_resolve builds PROBER_RESOLVED_BUILD from. Staging under
    # ci/prober/.build or ci/.build instead of the repo root's .build would
    # rebuild a copy nothing ever reads, leaving the real staged .so exactly as
    # stale as before.
...
# written. The C-test binary is built by ci/prober/build.sh's t/*_test.c loop, so
...
# jstrn_test is built by ci/prober/build.sh's *_test.c discovery loop, so this
