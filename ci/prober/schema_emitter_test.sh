...
# One row per FIELDS entry plus the fixed sweeps at the end. A hand-kept count,
# so adding a field to FIELDS below means bumping this too -- the mismatch is
# caught (the runner fails a suite whose plan and run count disagree, with no
...
fds_by_kind.anon:anon
fds_by_kind.other:other
smaps:smaps
smaps.pss:pss
smaps.private_dirty:private_dirty
pool:pool
pool.cycle_used:cycle_used
pool.cycle_blocks:cycle_blocks
pool.cycle_large:cycle_large
pool.cycle_cleanup:cycle_cleanup
zone:zone
