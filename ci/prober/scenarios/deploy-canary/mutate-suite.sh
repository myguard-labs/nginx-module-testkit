#!/usr/bin/env bash
#
# The "suite" mutate.sh runs for the deploy-canary oracle claims, which live in driver.sh
# rather than in a compiled test binary.
# It is not a unit-test binary -- it is the scenario, run exactly the way a
# human runs it from ci/prober/ (see driver.sh's header for the by-hand recipe).
...
run_mutate_suite scenarios/deploy-canary nginx 1.29.0
