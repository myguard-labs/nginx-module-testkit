...
#
# Scenario: CONSUMER PER-REQUEST ALLOCATION NEUTRALITY for ngx_http_coraza_module.so.
...
#  2 a signature proving ngx_http_coraza_module.so's code path actually ran
...
if [ "$WARMUP_OK" -eq 1 ] && trigger_request "$HITCHECK" "X-Consumer-Probe: trip"; then
    if grep -qiE '^HTTP/1\.1 418' "$HITCHECK"; then
        SIG_SEEN=1
    fi
fi
if [ "$SIG_SEEN" -eq 1 ]; then
    echo "ok 2 - coraza's own rule DENIED with 418 (a status nginx never returns by itself; without the engine the request is a plain 200 -- verified both ways)"
else
    echo "not ok 2 - no signature that ngx_http_coraza_module.so ran was observed"
