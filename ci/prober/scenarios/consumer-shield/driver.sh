...
#
# Scenario: CONSUMER PER-REQUEST ALLOCATION NEUTRALITY for ngx_http_shield_module.so.
...
#  2 a signature proving ngx_http_shield_module.so's code path actually ran
...
if [ "$WARMUP_OK" -eq 1 ]; then
    trigger_request "$PROBER_PREFIX/prime.out" "User-Agent: /bin/sh" || true
fi
if [ "$WARMUP_OK" -eq 1 ] && trigger_request "$HITCHECK" "User-Agent: /bin/sh"; then
    if grep -qiE '^HTTP/1\.1 403' "$HITCHECK"; then
        SIG_SEEN=1
    fi
fi
if [ "$SIG_SEEN" -eq 1 ]; then
    echo "ok 2 - the shield module BLOCKED a request carrying an attack pattern with 403 (a benign request gets 200)"
else
    echo "not ok 2 - no signature that ngx_http_shield_module.so ran was observed"
