...
#
# Scenario: CONSUMER PER-REQUEST ALLOCATION NEUTRALITY for ngx_http_skel_module.so.
...
#  2 a signature proving ngx_http_skel_module.so's code path actually ran
...
if [ "$WARMUP_OK" -eq 1 ]; then
    trigger_request "$PROBER_PREFIX/prime.out" "User-Agent: skel-marker" || true
fi
if [ "$WARMUP_OK" -eq 1 ] && trigger_request "$HITCHECK" "User-Agent: skel-marker"; then
    if grep -qiE '^HTTP/1\.1 403' "$HITCHECK"; then
        SIG_SEEN=1
    fi
fi
if [ "$SIG_SEEN" -eq 1 ]; then
    echo "ok 2 - the skeleton module BLOCKED a marker request with 403 (the same request without the marker gets 200 -- verified both ways)"
else
    echo "not ok 2 - no signature that ngx_http_skel_module.so ran was observed"
