...
#
# Scenario: CONSUMER PER-REQUEST ALLOCATION NEUTRALITY for ngx_http_zstd_filter_module.so.
...
#  2 a signature proving ngx_http_zstd_filter_module.so's code path actually ran
...
if [ "$WARMUP_OK" -eq 1 ]; then
    trigger_request "$PROBER_PREFIX/prime.out" "Accept-Encoding: zstd" || true
fi
if [ "$WARMUP_OK" -eq 1 ] && trigger_request "$HITCHECK" "Accept-Encoding: zstd"; then
    if grep -qiE 'Content-Encoding: zstd' "$HITCHECK"; then
        SIG_SEEN=1
    fi
fi
